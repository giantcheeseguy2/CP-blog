...
const template3 = '<div class="navbar"> <img src="../images/logo.jpg" class="logo"> <div class="title"><b>Oursaco</b> Blog</div> <button class="button" onclick="window.location.href=\'../index.html\'"><a href="../index.html">Home</a></button> <button class="button" onclick="window.location.href=\'../about.html\'"><a href="../about.html">About</a></button> <button class="button" onclick="window.location.href=\'../links.html\'"><a href="../links.html">Links</a></button> </div> <div style="width:100%;height:15%;"></div> <div class="blog">';
const template4 = '</div> <div class="tag-container">';
const template5 = '</div> <div class="blog">';
const template6 = '</div> </body> </html>';

const query = 'Who is best girl (lower case with spaces)';
const answer = '"qiao ling"';
const pwcheck = '<script>var password =' + answer + ';(function pwcheck(){var input = prompt("' + query + '"); while (input != password) { alert("Incorrect"); return pwcheck(); }}()); </script>';
    ...
    files.forEach(file => {
        fs.readFile(read + file, 'utf8', (err, data) => {
            ...
            for(let i = 0; i < tags.length; i++){
                html_tags += '<div class="tag"><p>' + tags[i] + '</p></div>';
                protected = protected || (tags[i] == ' protected');
            }

            let value = template1 + title + template2 + (protected ? pwcheck : '') + template3 + writer.render(reader.parse('# ' + title)) + '<div style="font-family:Helvetica; font-size:15px; text-align:center; padding:10px;">' + date + '</div>' + template4 + html_tags + template5 + writer.render(reader.parse(content)) + template6;
